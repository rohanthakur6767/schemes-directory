...
- [x] **Deeper scheme content (D33):** `apply_steps` + `faqs` columns; generated by the
      prose step (one LLM call now emits prose + 3–7 steps + 3–6 FAQs); rendered as numbered
...
- **D33 — Deeper per-scheme content: apply_steps + FAQs** (2026-06-07). Stored as jsonb on
  `scheme_translations`; one LLM call (prose step) emits prose + ordered steps + Q&A pairs.
  Rendered as numbered steps + `<details>` FAQ accordions + **FAQPage JSON-LD** (rich-result
  eligible). Review tool edits both (steps = one-per-line; FAQs = "question | answer" per line).
